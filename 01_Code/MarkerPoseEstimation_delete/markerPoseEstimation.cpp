	...
	public:
		...
		bool solvePnPOwn(const std::vector<cv::Point3f>& objectPoints,
			...
			for (size_t i = 0; i < objectPoints.size(); i++){
				...
				A.at<double>(2 * i, 3) = -objectPoints[i].x;
				A.at<double>(2 * i, 4) = -objectPoints[i].y;
				A.at<double>(2 * i, 5) = -z_value;
				A.at<double>(2 * i, 6) = normalizedImagePoints[i].y * objectPoints[i].x;
				A.at<double>(2 * i, 7) = normalizedImagePoints[i].y * objectPoints[i].y;
				A.at<double>(2 * i, 8) = normalizedImagePoints[i].y * z_value;

				A.at<double>(2 * i + 1, 0) = objectPoints[i].x;
				A.at<double>(2 * i + 1, 1) = objectPoints[i].y;
				A.at<double>(2 * i + 1, 2) = z_value;
				A.at<double>(2 * i + 1, 3) = 0.0;
				A.at<double>(2 * i + 1, 4) = 0.0;
				A.at<double>(2 * i + 1, 5) = 0.0;
				A.at<double>(2 * i + 1, 6) = -normalizedImagePoints[i].x * objectPoints[i].x;
				A.at<double>(2 * i + 1, 7) = -normalizedImagePoints[i].x * objectPoints[i].y;
				A.at<double>(2 * i + 1, 8) = -normalizedImagePoints[i].x * z_value;*/
			}
			...
			// Estimate rotation vector and translation vector from h
			cv::Rodrigues(h(cv::Rect(0, 0, 3, 3)), rvec);
			tvec = h(cv::Rect(2, 0, 1, 3));*/
